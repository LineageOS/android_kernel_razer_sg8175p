...
extern unsigned int focaltch_exist;
	...
	struct goodix_ts_board_data *board_data)
{
	...
	r = of_get_named_gpio(node, "focaltech,reset-gpio", 0);
	if (r < 0) {
	...
	r = of_get_named_gpio(node, "focaltech,irq-gpio", 0);
	if (r < 0) {
	...
	r = of_property_read_string(node, "focaltech,avdd-name", &name_tmp);
	if (!r) {
	...
	r = of_property_read_string(node, "focaltech,iovdd-name", &name_tmp);
	if (!r) {
	...
	max_x = board_data->panel_max_x;
	max_y = board_data->panel_max_y;
	...
	for (i = 0; i < GOODIX_MAX_TOUCH; i++) {
		if (touch_data->coords[i].status == TS_TOUCH) {
			...
			input_report_abs(dev, ABS_MT_POSITION_X,
					max_x - 1 - touch_data->coords[i].x);
			input_report_abs(dev, ABS_MT_POSITION_Y,
					max_y - 1 - touch_data->coords[i].y);
...
static int goodix_ts_check_dt(struct device_node *np)
{
    int i;
    int count;
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	if (!bus_interface) {
	 ...
	 dp = bus_interface->dev->of_node;//spi->dev.of_node;
	 //ts_info("Touch Screen(SPI BUS) driver prboe...");
	 ret = goodix_ts_check_dt(dp);
	 if (ret == -EPROBE_DEFER)
		 return ret;
	...
	hq_register_hw_info(HWID_CTP, "goodix_gt9916_visionox_20220209");
	return 0;
...
static int __init goodix_ts_core_init(void)
{
	...
	if (1 == focaltch_exist) {
		ts_info("Focaltch is already exist!");
		return ret;
	}

	ts_info("Core layer init:%s, focaltch_exist:%d", GOODIX_DRIVER_VERSION, focaltch_exist);
